...
public sealed class ExportImportService(ChoboDbContext db)
{
    ...
    public async Task<ExportEnvelope> ExportAsync(bool configOnly)
    {
        ...
        var payload = new ExportPayload(
            ...
            clusters.Select(x => new ClusterExport(x.Id, x.Name, x.Mode, x.AccessNodes.Select(n => new AccessNodeDto(n.Id, n.Host, n.Port, n.UseTls)).ToList(), x.EncryptedUserName, x.EncryptedPassword, x.IsDeleted, x.CreatedAt, x.UpdatedAt, x.DeletedAt)).ToList(),
            ...
            policies.Select(x => new BackupPolicyExport(x.Id, x.Name, x.SourceClusterId, x.SelectorJsonVersion, JsonSerializer.Deserialize<PolicySelector>(x.SelectorJson, JsonOptions)!, x.IsDeleted, x.CreatedAt, x.UpdatedAt, x.DeletedAt)).ToList(),
    ...
    public async Task ImportAsync(ExportEnvelope envelope, bool configOnly)
    {
        ...
        try
        {
            ...
            foreach (var cluster in envelope.Data.Clusters)
            {
                ...
                {
                    Id = cluster.Id, Name = cluster.Name, Mode = cluster.Mode, EncryptedUserName = cluster.EncryptedUserName, EncryptedPassword = cluster.EncryptedPassword,
            ...
            db.BackupPolicies.AddRange(envelope.Data.BackupPolicies.Select(x => new BackupPolicyEntity { Id = x.Id, Name = x.Name, SourceClusterId = x.SourceClusterId, SelectorJsonVersion = x.SelectorJsonVersion, SelectorJson = JsonSerializer.Serialize(x.Selector, JsonOptions), IsDeleted = x.IsDeleted, CreatedAt = x.CreatedAt, UpdatedAt = x.UpdatedAt, DeletedAt = x.DeletedAt }));
