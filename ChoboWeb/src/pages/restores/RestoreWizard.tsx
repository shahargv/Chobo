    ...
    <Page title="Start restore" subtitle="Choose a backup, destination cluster, tables, and shard layout for a restore." action={<Link className="secondary" to="/restores"><ArrowLeft size={16} /> Restore history</Link>}>
      <section className="restore-workbench">
        <div className="restore-main panel">
          ...
          {selectedBackupSummary?.encryptionState === "EncryptedMissingKey" && <div className="warning-panel" role="alert">This backup has protected shards whose AES keys are unavailable. You may continue with a table/shard selection only when every selected protected shard shows an available key; the server will reject unusable selections and list their shard and key IDs.</div>}
