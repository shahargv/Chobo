...
import { RefreshCw, RotateCcw, Save, SlidersHorizontal, Trash2 } from "lucide-react";
...
export function SettingsPage() {
  ...
  const visible = items.filter((item) => !section || item.section === section);
  const setSetting = useMutation({
    mutationFn: ({ key, value }: { key: string; value: string }) => api.setRuntimeSetting(key, value),
    onSuccess: (result) => {
      showToast({ kind: result.restartRequired ? "success" : "success", text: result.restartRequired ? "Setting saved. Restart required." : "Setting saved." });
      settings.refetch();
    },
  ...
  const unsetSetting = useMutation({
    ...
    onSuccess: (result) => {
      showToast({ kind: result.restartRequired ? "success" : "success", text: result.restartRequired ? "Overlay removed. Restart required." : "Overlay removed." });
      settings.refetch();
    },
    onError: (error) => showToast({ kind: "error", text: String(error) })
  });
    ...
    <section className="panel settings-panel">
      <div className="section-head">
        <div>
          ...
          <p>Environment or command-line values override saved overlay edits.</p>
        </div>
      ...
      <DataTable headers={["Setting", "Value", "Overlay", "Apply", "State", "Edit", "Actions"]} isLoading={settings.isLoading}>
        {visible.map((item) => {
          ...
          return <tr key={item.key}>
            ...
            <td>{item.hasOverlayValue ? <code>{formatValue(item.overlayValue)}</code> : <span className="muted">Default</span>}</td>
            <td><Status value={item.applyMode === "Live" ? "Live" : "Restart required"} /></td>
            <td>{item.isExternallyOverridden ? <Status value="Overridden" /> : item.hasOverlayValue ? <Status value="Overlay" /> : <span className="muted">Default</span>}</td>
            <td><SettingEditor setting={item} value={draft} onChange={(value) => setDrafts((current) => ({ ...current, [item.key]: value }))} /></td>
            <td><div className="actions"><button className="primary" disabled={setSetting.isPending || item.isReadOnly} title="Save setting" onClick={() => setSetting.mutate({ key: item.key, value: draft })}><Save size={15} /></button><button className="secondary" disabled={unsetSetting.isPending || !item.hasOverlayValue} title="Remove overlay" onClick={() => unsetSetting.mutate(item.key)}><Trash2 size={15} /></button><button className="ghost" title="Reset edit field" onClick={() => setDrafts((current) => ({ ...current, [item.key]: item.overlayValue ?? item.effectiveValue ?? "" }))}><RotateCcw size={15} /></button></div></td>
          </tr>;
        })}
      </DataTable>
      {!settings.isLoading && visible.length === 0 && <Empty text="No settings match the selected section." />}
    </section>
  </Page>;
}
...
function placeholder(setting: RuntimeSettingDto) {
  if (setting.valueType === "TimeSpan") return "00:00:02";
