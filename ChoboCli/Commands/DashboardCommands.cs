...
public sealed class DashboardCommands : CliSubject
{
    ...
    private static string Format(DashboardDto dashboard)
    {
        ...
        else
        {
            ...
            {
                builder.AppendLine($"  {schedule.ScheduleName}  enabled={schedule.IsEnabled}  policy={DisplayName(schedule.PolicyName, schedule.PolicyId)}  last={FormatOptionalTime(schedule.LastRunAt)}  status={schedule.LastRunStatus?.ToString() ?? "never"}  lastSuccess={FormatOptionalTime(schedule.LastSuccessfulRunCompletedAt)}");
            }
        }
